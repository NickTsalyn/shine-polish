...
const DateTimeCleaning: React.FC<DateTimeCleaningProps> = ({form}) => (
 <div>
  <p className="text-accent text-[24px] lg:text-[28px] text-center lg:text-start">
   {form.selectedDate ? (
    <>
     Your cleaning date is: <span className="text-[#DE005D]">{form.selectedDate} </span>
    </>
   ) : (
   ...
   {form.time && (
    <>
     <br className="hidden md:block lg:hidden" /> at <span className="text-[#DE005D]">{form.time}</span>
    </>
   )}
  </p>
 </div>
