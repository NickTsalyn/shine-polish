...
const Step1: React.FC<StepProps> = ({control, setStepCompleted}) => {
 ...
 return (
  <div className="py-4 md:py-6 lg:py-9 lg:min-h-[600px] xl:h-[1000px] flex flex-col gap-6 lg:gap-x-10 lg:gap-y-0 lg:grid lg:grid-flow-col lg:grid-cols-2 justify-between ">
   {/* maincontainer - first div */}
   <div className="md:flex md:flex-row lg:flex-col md:justify-between md:gap-6 lg:gap-[32px] xl:gap-[30px] lg:row-span-2 lg:h-[600px] lg:w-[576px] ">
      ...
      <Controller
       ...
       render={({field, fieldState: {error}}) => (
        <>
         ...
         />
         {error && (
          <p className="text-accent-light text-xs lg:text-base pl-2  left-2 bottom-[-15px] ">{error.message}</p>
         )}
        </>
       )}
      />
    ...
    <div className="hidden md:block md:min-w-[400px] md:max-w-[800px] md:h-[330px] lg:w-[576px] lg:h-[400px] xl:w-[758px] xl:h-[450px] relative">
     <Image
      src={img_stub}
      alt="map"
      layout="fill"
   ...
   <div className="flex flex-col lg:h-[600px] lg:w-[576px] justify-between">
    ...
    <div className="flex flex-col gap-4 md:gap-6 lg:col-span-1 mt-auto lg:w-[576px]">
     ...
     <ul className="flex flex-wrap justify-center gap-5 lg:gap-6 lg:w-[576px]  md:justify-around md:flex-nowrap lg:flex-wrap lg:justify-between">
      {frequency.map(({value, label}) => {
       return (
        <li
         key={value}
         className="flex justify-center items-center w-[132px] md:min-w-[160px] lg:min-w-[276px]"
        >
         <Controller
          name="frequency"
          control={control}
