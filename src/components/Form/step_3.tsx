  ...
  >
   <div className="py-4 md:py-6 lg:py-9">
    ...
    <div className="flex flex-col gap-2 md:gap-5 lg:gap-7">
     <div className="md:flex md:flex-row gap-6  md:h-10 lg:gap-[40px] lg:h-[48px] lg:w-auto">
      <div className="md:w-1/2 md:h-[48px] lg:w-3/5 relative mb-4">
       <Input
        name="name"
        value={localName}
        type="text"
        ...
        style="form-input"
       />
       {errors.name && <p className="text-secondary text-xs  absolute left-2 bottom-[-15px]">{errors.name}</p>}
      </div>
      <div className="md:w-1/2 lg:w-2/5 relative">
       <Input
        ...
        style="form-input"
       />
       {errors.surname && <p className="text-secondary text-xs  absolute left-2 bottom-[-15px]">{errors.surname}</p>}
      </div>
     </div>

     <div className="flex flex-col lg:flex-row-reverse gap-2 md:gap-5 lg:gap-[60px] lg:h-[48px]">
      <div className="lg:w-3/5 lg:h-[48px] relative">
       <Input
        name="email"
        value={localEmail}
        type="email"
       ...
       />
       {errors.email && (
        <p className="text-secondary text-xs  absolute left-2 bottom-[-6px] lg:bottom-[-16px]">{errors.email}</p>
       )}
      </div>
      <div className="lg:w-2/5 lg:h-[48px] relative">
       <IMaskInput
       ...
       />
       {errors.phone && (
        <p className="text-secondary text-xs  absolute left-2 bottom-[-6px] lg:bottom-[-16px]">{errors.phone}</p>
       )}
      </div>
     </div>
    </div>
   ...
   <div className="flex flex-col gap-6 lg:relative">
    <div className="lg:flex lg:flex-row-reverse lg:gap-[60px]">
     ...
     <div className="flex flex-col lg:w-1/2">
      <h2 className="mb-4 md:mb-5 lg:mb-[26px] text-2xl md:text-4xl font-medium md:font-normal">
       Anything else we should know to provide the best and polish cleaning possible?
      </h2>
    ...
    <div className="flex items-center gap-2 md:gap-5 lg:absolute lg:right-0 lg:top-[240px]">
     ...
     <p className="text-secondary text-xs md:text-2xl">Send me reminders about my booking via email</p>
    </div>
    <div className="ml-auto relative hover:text-white lg:absolute lg:right-0 lg:bottom-0">
     <Button
      style="apply-btn-light-one"
      type="submit"
     >
      CONFIRM
     </Button>
