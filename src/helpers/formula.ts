...
export const getPrice = (
  ...
  const areaCoefficient =
    areas.find((ar) => ar.name === selectedArea)?.value || 1;
  const discountValue =
    discount.find((discount) => discount.name === discountType)?.value || 1;
  const cleaningValue =
    serviceOption.find((serv) => serv.name === selectedService)?.value || 1;
  ...
  const basePrice =
    base * (1.1 + (bedrooms - 2) * coff) + bathrooms * bathPrice;
  const finalPrice =
    basePrice * areaCoefficient * discountValue * cleaningValue + extraValue;
  return finalPrice;
};
